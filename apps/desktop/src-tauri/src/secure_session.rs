...
pub fn secure_session_get() -> Result<Option<String>, String> {
    platform::get()?.map(String::from_utf8).transpose().map_err(|_| "invalid stored token".into())
}
...
mod platform {
    ...
    fn target() -> Vec<u16> {
        std::ffi::OsStr::new(SERVICE).encode_wide().chain(Some(0)).collect()
    }
    ...
    pub fn set(secret: &[u8]) -> Result<(), String> {
        ...
        if written == 0 { Err(std::io::Error::last_os_error().to_string()) } else { Ok(()) }
    }
    ...
    pub fn get() -> Result<Option<Vec<u8>>, String> {
        ...
        if read == 0 {
            ...
            return if error.raw_os_error() == Some(ERROR_NOT_FOUND) { Ok(None) } else { Err(error.to_string()) };
        }
    ...
    pub fn delete() -> Result<(), String> {
        ...
        let error = std::io::Error::last_os_error();
        if error.raw_os_error() == Some(ERROR_NOT_FOUND) { Ok(()) } else { Err(error.to_string()) }
    }
}
...
mod platform {
    ...
    extern "C" {
        fn SecKeychainFindGenericPassword(keychain: *const c_void, service_len: u32, service: *const u8, account_len: u32, account: *const u8, password_len: *mut u32, password_data: *mut *mut c_void, item: *mut ItemRef) -> i32;
        fn SecKeychainAddGenericPassword(keychain: *const c_void, service_len: u32, service: *const u8, account_len: u32, account: *const u8, password_len: u32, password_data: *const c_void, item: *mut ItemRef) -> i32;
        fn SecKeychainItemModifyAttributesAndData(item: ItemRef, attributes: *const c_void, length: u32, data: *const c_void) -> i32;
    ...
    fn find(password_len: &mut u32, password_data: &mut *mut c_void, item: &mut ItemRef) -> i32 {
        unsafe { SecKeychainFindGenericPassword(ptr::null(), SERVICE.len() as u32, SERVICE.as_ptr(), ACCOUNT.len() as u32, ACCOUNT.as_ptr(), password_len, password_data, item) }
    }
    ...
    pub fn set(secret: &[u8]) -> Result<(), String> {
        ...
        if status == 0 {
            ...
            let modified = unsafe { SecKeychainItemModifyAttributesAndData(item, ptr::null(), secret.len() as u32, secret.as_ptr().cast()) };
            unsafe { CFRelease(item) };
            return if modified == 0 { Ok(()) } else { Err(format!("Keychain error {modified}")) };
        }
        if status != ERR_ITEM_NOT_FOUND { return Err(format!("Keychain error {status}")); }
        let added = unsafe { SecKeychainAddGenericPassword(ptr::null(), SERVICE.len() as u32, SERVICE.as_ptr(), ACCOUNT.len() as u32, ACCOUNT.as_ptr(), secret.len() as u32, secret.as_ptr().cast(), ptr::null_mut()) };
        if added == 0 { Ok(()) } else { Err(format!("Keychain error {added}")) }
    }
    ...
    pub fn get() -> Result<Option<Vec<u8>>, String> {
        ...
        let mut item = ptr::null_mut();
        let status = find(&mut length, &mut data, &mut item);
        if status == ERR_ITEM_NOT_FOUND { return Ok(None); }
        if status != 0 { return Err(format!("Keychain error {status}")); }
    ...
    pub fn delete() -> Result<(), String> {
        ...
        let mut item = ptr::null_mut();
        let status = find(&mut length, &mut data, &mut item);
        if status == ERR_ITEM_NOT_FOUND { return Ok(()); }
        if status != 0 { return Err(format!("Keychain error {status}")); }
        unsafe { SecKeychainItemFreeContent(ptr::null(), data); }
        let deleted = unsafe { SecKeychainItemDelete(item) };
        unsafe { CFRelease(item) };
        if deleted == 0 { Ok(()) } else { Err(format!("Keychain error {deleted}")) }
    }
}

#[cfg(not(any(target_os = "windows", target_os = "macos")))]
mod platform {
    pub fn set(_: &[u8]) -> Result<(), String> { Err("secure storage is unsupported".into()) }
    pub fn get() -> Result<Option<Vec<u8>>, String> { Err("secure storage is unsupported".into()) }
