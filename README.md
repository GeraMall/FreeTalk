...
В текущей локальной сборке endpoint намеренно указывает на несуществующий безопасный адрес: публичное хранилище релизов ещё не создано. Поэтому кнопка проверки сейчас честно сообщает, что сервер недоступен. Для включения обновлений нужно опубликовать `latest.json`, подписанные bundles и `.sig` по HTTPS, затем заменить endpoint перед release-сборкой. Закрытый ключ создан только локально в игнорируемой папке `work/signing`; он не входит в исходники или установщик. Инструкция: [docs/UPDATES.md](docs/UPDATES.md).
